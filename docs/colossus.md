...
- `match-move-1m` / `match-move-5m`: keeps the opening book enabled, disables prediction, sets line-depth UI state high, and uses Colossus's own normal move-time state. Prefer these profiles for finished engine-vs-engine data.
